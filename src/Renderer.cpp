...
void Renderer::CreateComputeDescriptorSetLayout() {
	...
	VkDescriptorSetLayoutBinding numBladesSSBOLayoutBinding = {};
	numBladesSSBOLayoutBinding.binding = 0;
	numBladesSSBOLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
	numBladesSSBOLayoutBinding.descriptorCount = 1;
	numBladesSSBOLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
	numBladesSSBOLayoutBinding.pImmutableSamplers = nullptr;

	VkDescriptorSetLayoutBinding bladesSSBOLayoutBinding = {};
	bladesSSBOLayoutBinding.binding = 1;
	bladesSSBOLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
	bladesSSBOLayoutBinding.descriptorCount = 1;//
	bladesSSBOLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
	bladesSSBOLayoutBinding.pImmutableSamplers = nullptr;

	VkDescriptorSetLayoutBinding culledBladesSSBOLayoutBinding = {};
	culledBladesSSBOLayoutBinding.binding = 2;
	culledBladesSSBOLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
	culledBladesSSBOLayoutBinding.descriptorCount = 1;//
	culledBladesSSBOLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
	culledBladesSSBOLayoutBinding.pImmutableSamplers = nullptr;

	VkDescriptorSetLayoutBinding bucketBladeCountsSSBOLayoutBinding = {};
	bucketBladeCountsSSBOLayoutBinding.binding = 3;
	bucketBladeCountsSSBOLayoutBinding.descriptorType = VK_DESCRIPTOR_TYPE_STORAGE_BUFFER;
	bucketBladeCountsSSBOLayoutBinding.descriptorCount = 1;//
	bucketBladeCountsSSBOLayoutBinding.stageFlags = VK_SHADER_STAGE_COMPUTE_BIT;
	bucketBladeCountsSSBOLayoutBinding.pImmutableSamplers = nullptr;

	std::vector<VkDescriptorSetLayoutBinding> bindings = { 
		numBladesSSBOLayoutBinding, 
		bladesSSBOLayoutBinding, 
		culledBladesSSBOLayoutBinding, 
...
void Renderer::CreateDescriptorPool() {
    ...
    std::vector<VkDescriptorPoolSize> poolSizes = {
		...
		// Compute numBlades + Compute blades + Compute culledBlades + Compute bucketBladeCounts
		{ VK_DESCRIPTOR_TYPE_STORAGE_BUFFER , static_cast<uint32_t>(4 * scene->GetBlades().size()) },
		//{ VK_DESCRIPTOR_TYPE_STORAGE_BUFFER , static_cast<uint32_t>(2 * scene->GetBlades().size() * NUM_BLADES + scene->GetBlades().size()) },

		// Compute blades
		//{ VK_DESCRIPTOR_TYPE_STORAGE_BUFFER , static_cast<uint32_t>(scene->GetBlades().size()) },

		// Compute culledBlades
		//{ VK_DESCRIPTOR_TYPE_STORAGE_BUFFER , static_cast<uint32_t>(scene->GetBlades().size()) },
    };
...
void Renderer::CreateComputeDescriptorSets() {
	...
	for (uint32_t i = 0; i < scene->GetBlades().size(); ++i) {
		...
		bladesBufferInfo.range = VK_WHOLE_SIZE;// NUM_BLADES * sizeof(Blade);
		...
		culledBladesBufferInfo.range = VK_WHOLE_SIZE;// NUM_BLADES * sizeof(Blade);

		VkDescriptorBufferInfo bucketBladeCountsInfo = {};
		bucketBladeCountsInfo.buffer = scene->GetBlades()[i]->GetBucketBladeCounts();
		bucketBladeCountsInfo.offset = 0;
		bucketBladeCountsInfo.range = VK_WHOLE_SIZE;

		// Bind SSBO to the descriptor
		...
		descriptorWrites[4 * i + 0].pBufferInfo = &numBladesBufferInfo;
		...
		descriptorWrites[4 * i + 1].pBufferInfo = &bladesBufferInfo;
		...
		descriptorWrites[4 * i + 2].pBufferInfo = &culledBladesBufferInfo;
		...
		descriptorWrites[4 * i + 3].pBufferInfo = &bucketBladeCountsInfo;
...
void Renderer::CreateComputePipeline() {
    ...
    std::vector<VkDescriptorSetLayout> descriptorSetLayouts = { cameraDescriptorSetLayout, modelDescriptorSetLayout, timeDescriptorSetLayout, computeDescriptorSetLayout };
...
void Renderer::RecordComputeCommandBuffer() {
    ...
    vkCmdBindDescriptorSets(computeCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePipelineLayout, 2, 1, &timeDescriptorSet, 0, nullptr);

    // TODO: For each group of blades bind its descriptor set and dispatch
	for (uint32_t j = 0; j < scene->GetBlades().size(); ++j) {
		vkCmdBindDescriptorSets(computeCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePipelineLayout, 3, 1, &computeDescriptorSets[j], 0, nullptr);
		vkCmdBindDescriptorSets(computeCommandBuffer, VK_PIPELINE_BIND_POINT_COMPUTE, computePipelineLayout, 1, 1, &grassDescriptorSets[j], 0, nullptr);
		int workGroupCount = int(ceil((NUM_BLADES + WORKGROUP_SIZE - 1) / WORKGROUP_SIZE));

		vkCmdDispatch(computeCommandBuffer, workGroupCount, 1, 1);
	}
